...
bool MovieClip::createFromFile(char* fileName){
	...
	if(Manager::fileHandler.getFile(fileName, &newFile))
	{
		...
		Manager::fileHandler.fileToSram(&newFile, resolution, 8);
		
		uint16_t width = Convert::toUInt16(resolution); // Atlas'ın genişliği
		uint16_t height = Convert::toUInt16(resolution + 2); // Atlas'ın yüksekliği
		uint16_t widthPart = Convert::toUInt16(resolution + 4); // Görüntü parçasının genişliği
		uint16_t heightPart = Convert::toUInt16(resolution + 6); // Görüntü parçasının yüksekliği
		...
		if(height != 0 && width != 0 && heightPart != 0 && widthPart != 0 && widthPart <= width && heightPart <= height)
		{
			...
			//if(width * rowCount == (newFile.size - 10)) // Görüntü atlasının boyutları geçerli mi kontrol eder
			...
			//image += 10; // Atlasın ilk 10 byte'ı görüntü özelliklerine ait (genişlik yükseklik, görüntü parçasının genişlik ve yüksekliği)
			...
			uint16_t imageCountV = rowCount / rowCountPart; // Dikey görüntü sayısı
			uint8_t frameCount = imageCountH * imageCountV; // Toplam görüntü sayısı
			uint8_t maskType = RAM::read(image + 9);
				...
				for(uint16_t h = 0; h < imageCountH; h++) // Yatay görüntü sayısı kadar
				{
					...
					if(maskType == IMAGE_SELF_MASK){
						...
						if(currentFrame % 2 == 0){
							...
							RAM::writeArray(imagePart, &resolution[4], 4); // Görüntünün genişlik ve yüksekliğini ilk 4 byte'a yaz
					...
					for(uint16_t j=0; j < rowCountPart; j++)
					{
						for(uint16_t i=0; i < widthPart; i++)
						{
							...
							uint8_t data = RAM::read(&image[currentRow + widthPart * h + i] + 10);
...
void MovieClip::playMovie(){
	...
	for(uint8_t i = 0; i < remainingFrameCount; i++){
		...
		RAM::write32(frameChangeTimes + i, frame_change_time);
	}
